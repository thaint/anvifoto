...
class AE_Posts
{
     ...
     * @since 1.0
     */
    public function convert( $post_data, $thumbnail = 'medium_post_thumbnail', $excerpt = true) {
        $result = array();
    ...
    public function insert($args) {
        ...
        if($disable_plan && !isset($args['post_status'])) {// disable plan
            if($pending) { // pending post
                $args['post_status'] = 'pending';
            }else { // disable pending post
                ...
                if(is_taxonomy_hierarchical($tax_name)) {
                    ...
                    if (isset($args[$tax_name])) {
                        if(is_array($args[$tax_name])) {
                            $a = array();
                            foreach ($args[$tax_name] as $key => $value) {
                                if(isset($value['name'])) $a[] = $value['name'];
                            }
                            $args['tax_input'][$tax_name] = $a;                            
                        }else{
    ...
    public function nearbyPost($args) {
        ...
        if (isset($args['near_lat']) && $args['near_lat'] && $args['radius']) {
            ...
            $sql = "select GROUP_CONCAT(ID SEPARATOR ',')
                        ...
                        acos(sin(A.meta_value * 0.0175) * sin( $near_latitude * 0.0175) 
            ...
            */
            ...
            if(!empty($results)) {
                $args['post__in'] = explode(',', $results[0][0]);    
            }else{
